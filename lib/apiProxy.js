...
const cacheRules = [
    { pattern: /^\/vrrf\/poststadion\b/, freshTtl: 20000, staleTtl: 21600000 },
...
const minutesFromVrrfText = (value) => {
    const text = String(value || '').trim().toLowerCase();
    const match = text.match(/-?\d+/);
    if (match) return Number(match[0]);
    if (text === 'sofort' || text === 'now') return 0;
    return null;
};

const mapVrrfBoardRawDeparture = (item, row) => {
    const relativeTime = Array.isArray(row) && row.length >= 3 ? String(row[2] || '').trim() : '';
    const delay = typeof item.delay === 'number' ? Math.round(item.delay) : null;
    return {
        line: String(item.line_no || item.line || item.name || '').trim(),
        destination: String(item.destination || item.direction || item.route_end || '').trim(),
        time: String(item.time || item.sched_time || '').trim(),
        relativeTime,
        minutes: minutesFromVrrfText(relativeTime),
        delay,
        cancelled: Boolean(item.is_cancelled || item.is_partially_cancelled),
    };
};

const mapVrrfBoardFormattedDeparture = (row) => ({
    line: String(row[0] || '').trim(),
    destination: String(row[1] || '').trim(),
    time: String(row[2] || '').trim(),
    relativeTime: String(row[2] || '').trim(),
    minutes: minutesFromVrrfText(row[2]),
    delay: null,
    cancelled: false,
});
...
const vrrfBoardSortValue = (item) => {
    if (item && typeof item.minutes === 'number') return item.minutes;
    const timeMinutes = hhmmToMinutes(item && item.time);
    return timeMinutes === null ? Number.MAX_SAFE_INTEGER : timeMinutes;
};

const berlinTimeText = () => new Intl.DateTimeFormat('de-DE', {
...
const buildVrrfBoardResponse = (data) => {
    if (!data || data.error || (!Array.isArray(data.raw) && !Array.isArray(data.preformatted))) {
        throw new Error(data && data.error ? data.error : 'Invalid finalrewind board response');
    }

    const rows = Array.isArray(data.preformatted) ? data.preformatted : [];
    const departures = Array.isArray(data.raw)
        ? data.raw
            .map((item, index) => mapVrrfBoardRawDeparture(item || {}, rows[index]))
            .filter(item => item.line || item.destination || item.time || item.relativeTime)
        : rows
            .filter(row => Array.isArray(row) && row.length >= 3)
            .map(mapVrrfBoardFormattedDeparture)
            .filter(item => item.line || item.destination || item.time);
    departures.sort((a, b) => vrrfBoardSortValue(a) - vrrfBoardSortValue(b));
    ...
    return {
        ...
        source: {
            ...
            backend: VRRF_BACKEND,
            stop: 'poststadion',
            url: `https://${VRRF_HOST}/poststadion.json?backend=${encodeURIComponent(VRRF_BACKEND)}`,
            version: data.version || null,
        },
        fetchedAt: new Date().toISOString(),
        updatedTime: berlinTimeText(),
    };
};

const requestVrrfBoard = (parsedUrl, callback) => {
    const noLines = Math.min(10, Math.max(1, Number(parsedUrl.searchParams.get('results') || parsedUrl.searchParams.get('no_lines') || 8)));
    const path = `/poststadion.json?backend=${encodeURIComponent(VRRF_BACKEND)}&no_lines=${noLines}`;
    const options = {
        hostname: VRRF_HOST,
        port: 443,
        path,
    ...
    const vrrfReq = https.request(options, (vrrfRes) => {
        ...
        vrrfRes.on('end', () => {
            try {
                ...
                const json = JSON.parse(body.toString('utf8'));
                callback(null, buildVrrfBoardResponse(json));
            } catch (e) {
                callback(e);
            }
        });
    ...
    vrrfReq.on('timeout', () => {
        vrrfReq.destroy(new Error(`${VRRF_HOST} board timeout`));
    });
    vrrfReq.on('error', callback);
    vrrfReq.end();
};
...
const handleApiProxy = (req, res, parsedUrl) => {
    ...
    if (proxyPathname === '/vrrf/poststadion') {
        if (cached && now - cached.fetchedAt <= cacheRule.freshTtl) {
            sendCachedResponse(req, res, cached, 'HIT');
            return;
        }

        requestVrrfBoard(parsedUrl, (vrrfError, data) => {
            if (vrrfError) {
                console.error(`[Proxy Error] ${VRRF_HOST}: ${errorText(vrrfError)}`);
                if (vrrfError && vrrfError.stack) console.error(vrrfError.stack);
                ...
                res.end(JSON.stringify({ error: 'VRRF proxy error', detail: errorText(vrrfError) }));
                return;
            }
