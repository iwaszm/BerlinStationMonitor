...
const models = {
    123: require('../models/bus123-poststadion-model.json'),
    142: require('../models/bus142-poststadion-model.json'),
};
...
const LINE_NAMES = Object.keys(models);
...
const lookupMean = (model, table, key) => {
...
const normalizeDirection = (model, departure, stopovers, postIndex) => {
    ...
    const lastStopMatch = modelDirections.find(direction => directionMatches(lastStopName, direction));
    if (lastStopMatch) return lastStopMatch;

    if (postIndex >= 0) {
        const postSequence = String(postIndex + 1);
        const sequenceMatch = modelDirections.find(direction => {
            const counts = model.directions[direction] && model.directions[direction].postSequenceCounts;
            return counts && counts[postSequence];
        });
...
const predictDelay = (model, { direction, dow, hour, prevDelay }) => {
    ...
    for (const [basis, key] of lookups) {
        const entry = lookupMean(model, basis, key);
        if (entry) {
            return {
...
const buildPrediction = async (departure) => {
    const lineName = departure && departure.line && departure.line.name;
    const model = models[lineName];
    ...
    const direction = normalizeDirection(model, departure, stopovers, index);
    ...
    const prediction = predictDelay(model, features);
...
const handlePoststadionPrediction = async (req, res, parsedUrl) => {
    ...
    try {
        ...
        const departures = (data.departures || []).filter(item => item && item.line && LINE_NAMES.includes(item.line.name) && item.tripId);
        const pairs = await Promise.all(departures.map(buildPrediction));
        const predictions = {};
        pairs.filter(Boolean).forEach(([key, value]) => {
        ...
        toJson(res, 200, {
            ...
            modelVersions: Object.fromEntries(Object.entries(models).map(([line, item]) => [line, item.modelVersion])),
